            ...
            if(email) {
                ...
                createMember({email: email, username: email}).then((response) => {
                    ...
                    if(response.message && 'success' === response.status){
                        // getEl(config.twwRegistrationFree).appendChild(successDiv(response.message));
                    ...
                    if(response.data && response.data.id && response.data.latest_txn) {
                        // getEl(config.twwRegistrationFree).appendChild(successDiv('You have successfully registered. Please check your email for your password.'));

                        getEl(config.twwRegistrationFree).appendChild(successDiv(response.message + ' Reloading page.'))
...
const createMember = async (data) => {
    const response = await fetch(`${state.endpoints.restUrl}/create-member`, {
        method: 'POST',
        headers: {
