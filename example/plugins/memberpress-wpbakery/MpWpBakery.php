...
class MpWpBakery {
  ...
  public function shortcode( $output, $shortcode, $prepared_atts, $tag ) {
    ...
    $mepr_rule = ! empty( $rule ) ? new MeprRule( $rule ) : false;

    if ( empty( $mepr_rule->ID ) || current_user_can( 'mepr-active', "rule: {$rule}" ) ) {
      return $output;
    }
    ...
    if ( 'custom' === $prepared_atts['memberpress_unauthorized'] && ! empty( $prepared_atts['memberpress_unauthorized_message'] ) ) { // Row setting
      $content = wp_kses_post( $prepared_atts['memberpress_unauthorized_message'] );
    } else if ( ! empty( $post_unauth_settings->unauth_message ) && 'custom' === $post_unauth_settings->unauth_message_type ) { // Post setting
      $content = $post_unauth_settings->unauth_message;
    } else if ( ! empty( $global_message ) ) { // Global setting
      $content = $global_message;
    } else { // Fallback
      $content = __( 'You do not have permission to view this content.', 'memberpress', 'memberpress-wpbakery' );
    }
