...
function pmb_pro_print_service_only( $explanation = '' )
{
    ...
    $hover_text = '<b><a href="' . $upgrade_url . '">' . __( 'Purchase Required', 'print-my-blog' ) . '</a></b>';
    if ( $explanation ) {
...
function pmb_map_section_to_division( ProjectSection $section )
{
    ...
    return apply_filters( 'pmb_map_section_to_division', pmb_map_section_height_to_division( $section->getHeight() ), $section );
}
 ...
 * @param array $context
 */
function pmb_render_template( $template_name, $context = array() )
{
    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- outputting HTML
 ...
 * @param array $context
 */
function pmb_get_contents( $filename, $context = array() )
{
...
{
    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- outputting HTML
    pmb_render_template( 'partials/design_preview.php', [
        'design' => $design,
    ] );
}
...
">
    ...
    ?>
                <span class="pmb-project-item-title-text"><?php 
    echo  esc_html( $title ) ;
    ?></span>
                <?php 
    do_action(
        'pmb_content_items__project-item-title end',
        $id,
        $title,
        $post_type,
        $template,
    ...
    if ( $post_type->name !== CustomPostTypes::CONTENT ) {
        ?><span tabindex="0" class="dashicons dashicons-update pmb-icon pmb-disabled-icon" title="<?php 
        echo  esc_attr( esc_html__( 'Upgrade to Professional License for one-click copying to Print Materials for customization.', 'print-my-blog' ) ) ;
        ?>"></span>
                                <?php 
    }
    ...
    ?>"
                                class="pmb-add-item" href="#pmb-content-item-<?php 
    echo  esc_attr( $id ) ;
    ?>"><span
    ...
    echo  pmb_section_template_selector( $template, $project ) ;
    ?></span>
                </div>
            </div>
        </div>

        <div class="pmb-nested-sortable pmb-draggable-area <?php 
    echo  ( $depth < $max_nesting ? 'pmb-sortable' : 'pmb-sortable-inactive' ) ;
    ?> pmb-subs">
            <?php 
    foreach ( $subs as $sub ) {
    ...
    <div class="pmb-no-sort pmb-drag-here no-drag">
        <div class="pmb-drag-here-inner">
            <a class="pmb-add-material" href="#ui-id-1">
                <?php 
    esc_html_e( 'Drag or click here', 'print-my-blog' );
    ?> <span class="pmb-add-section dashicons
